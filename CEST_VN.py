...
class ReconCell(nn.Module):
    ...
    def mri_forward_op(self, u, coil_sens, sampling_mask):
        """
        Parameters:
        u: complex input image [N 2 Z H W]
        coil_sens: coil sensitivity map [N 2 C Z H W ] [N C Z H W 2]
        sampling_mask: [N Z H W]

        Returns:
        kspace of u with applied coil sensitivity and sampling mask
        """
        coil_imgs = complex_mul(coil_sens, u.unsqueeze(2))  # Nx2XCxZxHxW

        coil_imgs = r2c(coil_imgs, axis=1)
        Fu = torch_fft2c(coil_imgs)
        Fu = c2r(Fu, axis=1)  # Nx2xCxZxHxW

        mask = sampling_mask.unsqueeze(1)  # Nx1xZxHxW
        mask = mask.unsqueeze(1)  # Nx1x1xZxHxW

        kspace = mask * Fu  # Nx2xCxZxHxW
        return kspace
    ...
    def mri_adjoint_op(self, f, coil_sens, sampling_mask):
        ...
        Parameters:
        ----------
        f: multi channel kspace [N 2 C H W]
        coil_sens: coil sensitivity map [N 2 C Z H W]
        sampling_mask: [N 2 Z H W]
        ...
        f = r2c(f, axis=1)
        Finv = torch_ifft2c(mask * f)
        Finv = c2r(Finv, axis=1)  # Nx2xCxZxHxW
    ...
    def forward(self, inputs):
        u_t_1 = inputs['u_t']  # Nx2xZxHxW
        f = inputs['f']
        c = inputs['coil_sens']
        m = inputs['sampling_mask']

        u_t_2 = u_t_1.unsqueeze(2)  # Nx2x1xZxHxW
        pad = self.options['pad']
        u_t_real = u_t_2[:, 0, :, :, :, :]
        u_t_imag = u_t_2[:, 1, :, :, :, :]

        u_t_real = F.pad(u_t_real, [pad, pad, pad, pad, pad, pad], mode='constant', value=0)  # to do: implement symmetric padding
        ...
        Ru = u_k_T[:, :, 0, pad:-pad, pad:-pad, pad:-pad]  # Nx2xZxHxW
...
class CEST_VN(nn.Module):
    def __init__(self, **kwargs):
        super(CEST_VN, self).__init__()
