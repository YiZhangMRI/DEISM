...
def save_recon_2(global_step, recon_CESTVN, recon_CalibNet, recon_dir):
    name = "Recon_epoch{}.mat".format(global_step)
    recon_CESTVN = np.squeeze(recon_CESTVN)
    recon_CalibNet = np.squeeze(recon_CalibNet)
    sio.savemat(os.path.join(recon_dir, name), {'recon_CESTVN': np.array(recon_CESTVN), 'recon_CalibNet': np.array(recon_CalibNet)})

    
def save_recon_test(name, recon, recon_dir):
    # name = "Recon_Tumor_cao_epoch_{}.mat".format(epoch+1)
    recon = np.squeeze(recon)
    sio.savemat(os.path.join(recon_dir, name), {'recon_DEISM': recon})
    
def save_recon_test_1(name, recon_CESTVN, recon_CalibNet, recon_dir):
    recon_CESTVN = np.squeeze(recon_CESTVN)
    recon_CalibNet = np.squeeze(recon_CalibNet)
    sio.savemat(os.path.join(recon_dir, name), {'recon_CESTVN': np.array(recon_CESTVN), 'recon_DEISM': np.array(recon_CalibNet)})
